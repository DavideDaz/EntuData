...
Every time approaching a new topic of Machine Learning at Tokyo Data Science, our good practice is to  firstly cover the theoretical bases before to put it in practice. Even though getting the general idea of a particular method may come easy, the understanding of the implementation on the code and its real effect on the model often bear a certain level of abstraction.This is especially true if dealing with deep neural network in which the choice of a particular parameter or setup is integrated in a elaborated architecture and the real perception of its effect is often clouded by the complexity of the net.

I therefore decided to break down each topic by supporting the theory with a very simple exercise or application (that often correspond to the school assignment) in order to effectively clarify the 'maths' behind it.
...
Please note that some of these techniques, in particular the last two, are particularly conceived for applications to deep learning in cases with complex landscape loss functions (Like the one at the head of this post taken from [losslandscape](https://losslandscape.com/), which I strongly suggest to take a look at. Therefore, their use on a the quadratic function may not lead to the expected result, even though it still can clarify their concept.


## SGD with Momentum Rationale

The learning rate is a crucial parameter for the gradient descent, as it determines the dimension of the step to upload the weights after each iteration. The simple SGD employs a constant learning rate, this means that all the parameters are updated with the same meter independently from the "shape" of the loss function. This can result in a slow journey to the local minima as shown in the case of Figure 1, that consider a simple case of two weights. Note that the loss is represented with contour lines as a function of the two parameters.
<img src="{{ site.url }}{{ site.baseurl }}/OptimizerComparison/MomentumandSGD.png" alt="SGD and SGD with momentum">
<figcaption>Figure 1: SGD and SGD with momentum Ref.[1]</figcaption>
...
We can think of the gradient as a sort of vector, with its magnitude and a direction (actually opposite direction) pointing with a certain intensity to the local minima. It can be therefore favorable to take advantage of the momentum given by the previous iteration to speed up the learning process. At the same time we don't want to accelerate indefinitely and amplify too much the step size, as we can overshoot our minima target. Without going too much in deep with the parallel with the physical definition of momentum (whose clear explanation is in the [deep learning book](http://www.deeplearningbook.org/contents/optimization.html)), for the SGD with momentum implementation we mainly need two ingredients:
...
The result is a faster descent towards the local minima with a damped oscillation of the weights as in Figure 2.
...
We can now finally get into the exercise on the quadratic regression. In the **notebook** [here](https://github.com/DavideDaz/TokyoDataScience/blob/master/Assignments/Gradient%20Descent%20Assignment/Basis%20Neural%20Network%20-%20Quadratic%20-%20SGD%20with%20momentum.ipynb) I compared the performance of the simple SGD (using a unique batch of inputs) with the SGD with the momentum applied. You can also open the notebook on Google Colab by right-clicking on the Colab button on top and opening it in a new tab.
Two values of friction factor have been also employed to compare the performance.The two implementation have been initialized with the same weight values and run for the same number of epochs for a more clear comparison.
...
From Figure 5 we can also observe the effect of the friction factor that in case of 0.9 introduces a 10% decay for each v term. This is quickly dumped until reaching a steady velocity given by $$\epsilon g/(1-\alpha)$$.
...
The use of a high value of alpha from the beginning of the cycle can introduce a high oscillation of the loss function. For the initial steps the gradient is still far to be aligned towards the local minima, and this error is not suppressed enough by the friction factor.
...
As a result the effect of the excessive amplification is propagated over all the next iterations (Figure 7), resulting sometimes in a higher loss at the end of the 1000 iterations.
